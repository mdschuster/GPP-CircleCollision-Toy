...
int main()
{
    ...
    while(!myfile.eof()){
        //read header
        myfile>>header;
        if(strcmp(header.c_str(),"Circle")==0){
            float x,y,speedX,speedY,r;
            std::string name;
            float R,G,B;
            myfile>>name>>x>>y>>r>>speedX>>speedY>>R>>G>>B;
            auto* color=new float[3]{R,G,B};
            auto c=std::make_shared<Circle>(name,x,y,r,speedX,speedY,color);
            shapeMap[name]=c;
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        BeginDrawing();
            ...
            rlImGuiBegin();
                ...
                ImGui::Begin("GPP-CircleCollision-Toy Controls",nullptr,0);
